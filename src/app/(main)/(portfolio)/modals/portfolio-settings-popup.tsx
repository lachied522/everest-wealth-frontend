...
type FormData = Pick<PortfolioData, 'name'|'objective'|'flat_brokerage'|'active'|'international'|'preferences'>
...
export const FormSchema = z.object({
    ...
    active: z.number().array().transform((val) => val[0]).nullable(), // must be converted back to number
...
export default function PortfolioSettingsPopup() {
    ...
    async function onSubmit(values: z.infer<typeof FormSchema>) {
        ...
        const newValues: Partial<FormData> = Object.entries(values).reduce((obj, [key, value]) => {
            if (value!==currentPortfolio[key as keyof FormData]) {
                return {
                    ...obj,
                    [key]: value,
                }
        ...
        if (Object.keys(newValues).length > 0) {
            ...
            const response = await commitChanges(newValues);
            success = true;
        } else {
                            ...
                            <form onSubmit={form.handleSubmit(onSubmit)}>
                                ...
                                <ScrollArea className="h-[60vh] my-6">
                                    <div className="flex flex-col items-stretch justify-items-between gap-12 p-6">
                                        <div className="grid grid-cols-[0.75fr,1fr] items-center justify-between">
